    ...
    <div class="table-responsive">
        <table class="table table-striped">
            ...
            <tr id="seller-<%= id %>">
                ...
                <td>
                    <a href="/app/admin/showsellerprofile?user_id=<%= id %>" class="btn btn-primary" style="background-color: limegreen">Profile</a>
                </td>
                <td>
                    <button id="status-btn-<%= id %>"
                ...
                <td>
                    <button onclick="deleteSeller(<%= id %>)" class="btn btn-danger" style="background-color: red">Delete</button>
                </td>
            </tr>
            <%
                    }
...
<script>
    function toggleStatus(userId, currentStatus) {
        ...
        xhr.open("POST", "/app/admin/updatesellerstatus", true);
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if (responseJson.success) {
                    ...
                    var newClass = responseJson.status === '<%= Status.ACTIVE.name() %>' ? "btn-danger" : "btn-success";
                    var newText = responseJson.status === '<%= Status.ACTIVE.name() %>' ? "Block" : "Unblock";
                    var newColor = responseJson.status === '<%= Status.ACTIVE.name() %>' ? "red" : "limegreen";
                    button.className = "btn " + newClass;
                    button.style.backgroundColor = newColor;
                    button.textContent = newText;
                } else {
                    document.getElementById("error-alert").innerText = "Failed to update status.";
    ...
    function deleteSeller(userId) {
        if (confirm("Are you sure you want to delete this seller?")) {
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "/app/admin/deleteseller", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    var responseJson = JSON.parse(xhr.responseText);
                    if (responseJson.success) {
                        document.getElementById("seller-" + userId).remove();
                        document.getElementById("success-alert").innerText = "Seller deleted successfully.";
                        document.getElementById("success-alert").style.display = "block";
                    } else {
                        document.getElementById("error-alert").innerText = "Failed to delete seller.";
                        document.getElementById("error-alert").style.display = "block";
                    }
                }
            };
