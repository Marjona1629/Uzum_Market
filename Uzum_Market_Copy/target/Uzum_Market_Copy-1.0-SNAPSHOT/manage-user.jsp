        ...
        <table class="table table-striped">
            ...
            <%
                ...
                try {
                    ...
                    rs = stmt.executeQuery("SELECT user_id, user_name, user_email, user_phone, user_address, user_city, status FROM users WHERE role = 'CUSTOMER'  AND isDeleted = FALSE");
                    while (rs.next()) {
            ...
            %>
            <tr id="customer-<%= rs.getInt("user_id") %>">
                ...
                <td>
                    <a href="customerprofile.jsp?user_id=<%= rs.getInt("user_id") %>"
                ...
                <td>
                    <%-- Delete Button --%>
                    <button onclick="deleteCustomer(<%= rs.getInt("user_id") %>)" class="btn btn-danger"
...
<script>
    function toggleStatus(userId, currentStatus) {
        ...
        $.ajax({
            url: "update-customer-status.jsp",
            type: "POST",
    ...
    function deleteCustomer(userId) {
        $.ajax({
            url: "update-customer-role.jsp",
            type: "POST",
            data: { user_id: userId, action: "delete" }, // Send action 'delete'
            success: function (response) {
                if (response.success) {
                    $("#customer-" + userId).remove(); // Remove customer row from table
                    $("#success-alert").text("Customer deleted successfully.").show();
                } else {
                    $("#error-alert").text("Error deleting customer.").show();
                }
            },
            error: function (xhr, status, error) {
                $("#error-alert").text("Error: " + error).show();
